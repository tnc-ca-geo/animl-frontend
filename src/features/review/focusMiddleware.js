...
import { selectIterationOptions, selectIsAddingLabel } from '../loupe/loupeSlice';
...
const getActiveLabelIndices = (imageIndex, objects, opts) => {
  ...
  for (const [i, object] of objects.entries()) {
    let labels = !object.locked
      ? object.labels.filter((label) => label.validation === null || label.validation.validated)
      : !opts.skipLockedObjects
        ? [object.labels.find((label) => label.validation && label.validation.validated)]
        : [];
...
const findNextLabel = (delta, images, focusIndex, opts) => {
  ...
  const findNextLabelOnImage = (imageIndex) => {
    ...
    if (!opts.skipEmptyImages && objects.length === 0 && initialImageEvaluated) {
    ...
    return activeLabelIndices[nextIndex] ? activeLabelIndices[nextIndex] : null;
  };
