...
export async function call({ projId, request, input }) {
  ...
  try {
    ...
    const query = gql`
      ${params.template}
    `;
    return params.variables ? await graphQLClient.request(query, params.variables) : await graphQLClient.request(query);
  } catch (err) {
    // https://github.com/prisma-labs/graphql-request/issues/201
    throw JSON.parse(JSON.stringify(err)).response?.errors || err;
  }
}
